...
AI開発時に必要な詳細契約です。通常は `AI_CONTEXT.md` と `docs/context-routing.md` から必要な節だけ参照してください。

## Required roadmap

- `docs/開発予定.md` を現在の開発予定と優先順位の正とする。
- 通常タスクでは対象節を検索して読む。ロードマップ全体・優先順位・横断変更を判断するときは全文確認する。
