...
- `project_type` は省略時 `standard`。special版など実験系は別値を指定し、通常版と混同しない。
...
## 第7マイルストーン: special版の分離

`obakeno_sumika_special` は通常版とは別のプロジェクトルート、別データ、別テストで扱う。LLMや知識推論の実装はここでは行わず、混線防止のための識別子と置き場所だけを先に確定する。

| 区分 | 場所 | 内容 |
| --- | --- | --- |
| project root | `projects/obakeno_sumika_special/` | special版だけの `engine_project.json` と起動入口 |
| project type | `obakeno_sumika_special` | 通常版 `standard` と明確に区別する値 |
| data | `projects/obakeno_sumika_special/data/` | 知識、会話生成タスク、実行時メモリの初期JSON |
| tests | `tests/test_special_project_isolation.py` | special版が通常版の実行データを参照しないことを確認 |

```mermaid
flowchart LR
    Standard[standard project] --> StandardData[root json data]
    Special[obakeno_sumika_special project] --> SpecialData[projects/obakeno_sumika_special/data]
    Loader[manifest_loader] --> Standard
    Loader --> Special
```
